      ...
      if (this.#edit_obj !== undefined) {
        ...
        switch (this.#edit_mode) {
          ...
          case "SCALE": {
            ...
            let scale_mult_add_num = (this.#objects[this.#edit_obj].local.scale.clone().length() * (2.2 * dt * this.glob_speed_mult) / scale_mult_size.length())

            this.#objects[this.#edit_obj].local.scale.copy(
              this.#objects[this.#edit_obj].local.scale
                .clone()
                .add(direction.multiplyScalar(Number.isFinite(scale_mult_add_num) ? scale_mult_add_num : 0))
            );
